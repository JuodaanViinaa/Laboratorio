...
def count_resp(marks, trialStart, trialEnd, response):  # Count_per_trial
    """
    Counts responses between trial start and trial end. If a single marker identifies both the response to count and the
    response which begins the trial, then there will be an extra response counted. This can be corrected with the
    "subtract" option in the analysis list.\n
    :param marks: List of markers.
    :param trialStart: Mark for trial start.
    :param trialEnd: Mark for trial start end.
...
def resp_dist(marks, time, trialStart, trialEnd, response, bin_size, bin_amount, unit):
    ...
    between the end of the last bin and the trial end will be aggregated in a single extra bin.\n
    :param marks: List of markers.
    :param time: List with session time.
    :param trialStart: Trial-start marker.
    :param trialEnd: Trial-end marker
    ...
    """
    inicio = 0
    resp_por_ensayo = [0] * (bin_amount + 1)  # Generate a list with as many zeros as the parameter bin_amount dictates.
    ...
    if trialStart == trialEnd:  # This route is taken if there is a single marker for both trial-start and trial-end.
        for index, mark in enumerate(marks):
            if mark.value == trialStart and inicio == 0:
                ...
                # end time for each bin. The end of a bin corresponds with the start of the next.
...
def total_count(marks, response):
    ...
    :param marks: List of markers.
    :param response: Response of interest marker.
    :return: Int with the total occurences of the response.
    """
    contador = 0
    for n in range(len(marks)):
...
def lat_count(marks, time, trialStart, response, unit):
    """
    Counts the latency between the start of a trial and the first response of interest.\n
    :param marks: List of markers.
    :param time: List with session time.
    :param trialStart: Trial-start marker.
    :param response: Response of interest marker.
...
def template():
    """
    Prints a template to facilitate the analysis list declaration.
    """
    print("""
    analysis_list = [
    {"fetch": {"cell_row": 10,
...
class Analyzer:
    ...
    def __init__(self, fileName: str, temporaryDirectory: str, permanentDirectory: str, convertedDirectory: str,
        ...
        :param timeColumn: The column in which the marks are written in the individual .xlsx files.
        :param markColumn: The column in which the time is written in the individual .xlsx files.
    ...
    def converter(self):
        ...
        for ind, sjt in enumerate(self.subject_list):
            for ssn in self.session_list[ind]:
                ...
                rx = re.compile(r'^\d+\.\d+$')
                # This section will be useful if a case in which the arrays begin on a row different from the 11th or
    ...
    def analyze(self, workbook, sheetDict):
        """
        Main analysis function. Uses all other functions and methods to count the variables of interest and
        ...
        :param sheetDict: Dictionary created vy the create_sheets function.
        """
