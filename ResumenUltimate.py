...
for sujeto in sujetos:
    regexDatoBruto = re.compile(sujeto + r'\w+\d+')
    for datoTemp in dirTemp:
        if not regexDatoBruto.search(datoTemp):
            sujetosFaltantes.append(sujeto)
        elif regexDatoBruto.search(datoTemp):
            for i in sujetosFaltantes:
                if i == sujeto:
                    sujetosFaltantes.remove(sujeto)
            break
print(sujetosFaltantes)
